  ...
  <div class="article-body">
    ...
    <div v-else class="create-body">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <h style="font-size: 30px;">영화 리뷰 게시글 만들기</h>
        <button @click="submitArticle" class="mb-2 mini-btn">생성하기</button>
      </div>
      ...
      <div class="select-movies">
        <MovieCardSelect ></MovieCardSelect>
      </div>
    </div>
  </div>
</template>
...
import MovieFind from "@/components/movie/MovieFind.vue";
import MovieCardSelect from "@/components/movie/MovieCardSelect.vue";
...
const selectedMovie = articleStore.selectedMovie[0];
...
const submitArticle = () => {
  article.value.movie = articleStore.selectedMovie[0];
  ...
  if (!article.value.movie) {
    alertMessage("하나의 영화가 선택되어야 합니다.");
    return;
  }
